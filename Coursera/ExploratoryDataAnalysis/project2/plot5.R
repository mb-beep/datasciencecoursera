...
vehicleNEI <- vehicleNEI[vehicleNEI$fips == "24510",]

# Melt vehicleNEI and recast it to get total sum of motor vehicle related emissions.
meltedNEI <- melt(vehicleNEI, id = c("year"), measure.vars = c("emissions"));
castNEI <- dcast(meltedNEI, year ~ variable, sum);

# Plot using base system
png("./figures/plot5.png", bg = "transparent");
g <- ggplot(castNEI, aes(year, emissions)) + xlab("Year") + ylab("Emissions in tons") + ggtitle("Motor vehicle emissions vs years") + geom_line();
print(g);
dev.off();